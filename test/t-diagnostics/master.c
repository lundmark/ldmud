...
#if __EFUN_DEFINED__(check_compile)
        ({ "compile check returns error context without invoking master", 0,
           (:
               int before = sizeof(diagnostics);
               mixed *result = check_compile("/local_initializer.c");
               return valid_result(result) && !result[0] &&
                   sizeof(result[1]) && result[1][0][0] == "/local_initializer.c" &&
                   sizeof(diagnostics) == before &&
                   has_context(result, "local_initializer.c", 4, 17,
                       "error", "    int count = \"three\";", 7,
                       ({ "expected int", "got string" }));
           :)
        }),
        ({ "compile check returns warning context and still succeeds", 0,
           (:
               int before = sizeof(diagnostics);
               mixed *result = check_compile("/warning.c");
               return valid_result(result) && result[0] &&
                   sizeof(diagnostics) == before && has_context(result,
                       "warning.c", 4, 5, "warning", "    42;", 2, ({}));
           :)
        }),
#endif
